...
- 團隊開發作品，作為學習 前後端分離架構 與 電商平台核心功能 的實戰專案。
- 專注於 行程購買體驗，模擬實際旅遊平台的使用場景。
- 目前專案後端服務已暫停，前端仍透過 GitHub Pages 展示。

### 🔹 功能特色

- 行程列表瀏覽：使用者可瀏覽不同旅遊行程。
- 行程詳情頁：點擊行程後可查看詳細資訊。
- 購物流程設計（原本設計於後端 API 中，現已停用）：包含購物車、訂單流程。

### 🚀 Demo

[網站展示](https://tripeasycompany.github.io/frontend/#/)

### 🔹 技術實作

- 前端：

  - 使用 Vue 3 + Vite 建構，採用 option API 撰寫程式碼。
  - 元件化設計，提升可維護性與可重複使用性。
  - GitHub Pages 部署前端內容。
...
- 後端（原架構，現停用）：
  ...
  - 設計行程、訂單、用戶相關 API，模擬電商交易流程。

- 資料庫：
  - 使用 PostgreSQL，設計用戶、商品（旅遊行程）、訂單等資料表。
  - 強調資料正規化與關聯性，提升查詢效率。
